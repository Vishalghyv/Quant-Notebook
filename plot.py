...
def plot_short(data):
    ...
    plt.scatter(data.index[-100:], data['Close'][-100:], c=data['short'][-100:].replace({1: 'g', -1: 'r', 0: 'w'}), label='Short Signal', marker='^', s=20)
    plt.legend(loc='upper left')
    ...
    ax1.plot(data['macd'][-100:], color=color)
    ax1.tick_params(axis='y', labelcolor=color)
    ax2 = ax1.twinx()
    color = 'tab:blue'
    ax2.set_ylabel('Signal', color=color)
    ax2.plot(data['signal'][-100:], color=color)
    ax2.tick_params(axis='y', labelcolor=color)
    ax3 = ax1.twinx()
    color = 'tab:green'
    ax3.set_ylabel('Short Signal', color=color)
    ax3.scatter(data.index[-100:], data['short_entry'][-100:].replace({1: 1, -1: -1, 0: 0}), color=color)
    ax3.tick_params(axis='y', labelcolor=color)

    ax4 = ax1.twinx()
    color = 'tab:orange'
    ax4.set_ylabel('Short Signal', color=color)
    ax4.scatter(data.index[-100:], data['short_exit'][-100:].replace({1: 1, -1: -1, 0: 0}), color=color)
